...
- **2026-07-11 (grille tarifaire complète — catalogue admin)** :
  - ⚠️⚠️ **`accesPro=true` sur `/stock/produits/autocomplete` MASQUE la plupart des unités** : il ne renvoie que **2 lots/saveur** (Carton 6×1L idLot 3, Carton 18×0,35L idLot 13) = 12 réfs. **SANS `accesPro`** (ou `accesPro=false`) → **41 réfs**, TOUS les lots avec leur `idStockBouteille` : Unité (idLot 1), Carton 6 (3), **Carton 12 (idLot 4)**, Carton 9 (6), Carton 18 (13), Fût 20L. Ex. Cola-Chaï Carton 12×0,35L = **idStockBouteille 165533**. → pour exposer/commander tous les conditionnements (dont le 12×35cl voulu par GOA), **ne PAS filtrer sur accesPro**.
  - **Grille tarifaire lisible via `GET /parametres/grille-tarifaire/matrice?idClientType={id}`** (200). `POST …/liste` renvoie 500 quel que soit `typeTarif` essayé (CONTENANT/BOUTEILLE/…) → **inutilisable**, utiliser la matrice. Réponse `ModeleMatriceTarif` : `{ conditionnements[] , produits[] , typesClient[], contenants[]… }`. `conditionnements[]` = colonnes `{contenant{idContenant,libelle}, lot{idLot,libelle,quantite}}`. `produits[]` = `{ modeleProduit{idProduit,libelle}, tarifs[] }` ; chaque tarif = `{ id, modeleContenant{idContenant}, modeleLot{idLot}, modeleClientType{idClientType}, prixHT, horsDroits }` (libellés contenant/lot ABSENTS du tarif → les résoudre via `conditionnements[]`).
  - **matrice SANS idClientType → vide** (`typesClient:[]`, 0 tarif). Il faut appeler **par idClientType**. Seuls les types AVEC grille explicite renvoient des tarifs : **client PRO 17849** (6 tarifs Cola : 1,95/23,40/35,10/4,55/27,30/79) et **Distributeur 20735** (4 tarifs : 18,72/28,08/21,84 + 1 sans prix). Les sous-types sans grille propre (ex. **GMS 20680 → 0 tarif**) héritent de PRO. → pour reconstruire la grille complète : itérer les types (`/parametres/client/type`), `matrice` par type, garder les non-vides.

- **2026-07-14 (grille enfant + prix sur `/commande/enregistrer`)** — test live (devis client test 588074, supprimé) :
  - **`grilleTarifaire: { idClientType }` accepte un type ENFANT à grille propre** (20735 Distributeur) — stocké tel quel sur la commande (`grilleTarifaire.libelle: "Distributeur"`). La règle « toujours la racine » (§4) est donc TROP restrictive : la bonne valeur = **la grille du prix réellement appliqué** (résolue en remontant `idParent` jusqu'à un type qui a une grille).
  - **Easybeer GARDE le `prixUnitaireHTHorsRemise` envoyé** (test : 10,00 envoyé → 10,25 relu = 10 + consigne) — il **ne recalcule PAS** le prix depuis la grille au moment du devis. ⇒ l'intégrité tarifaire dépend du prix qu'ON envoie ; la grille de la commande est une métadonnée de contexte (affichage/facturation Easybeer).

- **2026-07-14 (remises au niveau des LIGNES sur `/commande/enregistrer`)** — tests live contrôlés (client test 825435, devis jetables) :
  - **La remise de la FICHE client NE s'applique PAS au devis/commande** : devis créé SANS champ `remise` sur la ligne (fiche `remise="12%"`) → `remiseTotale = 0`, prix plein. Pour qu'une remise apparaisse sur la commande, il FAUT la mettre **sur la ligne** (`elementsBouteilles[i].remise`).
  - **UNE SEULE remise par ligne, via le champ `remise` (string %) — DÉFINITIF.** La ligne `ModeleCommandeElement` expose `remise, remise2, typeRemise, typeRemise2, valeurRemise, prixLotHTRemise`, mais tests live contrôlés : Easybeer calcule le rabais à partir de **`remise` UNIQUEMENT**. Les autres sont STOCKÉS tels quels mais **ignorés au calcul** ou en **sortie seule** :
    - `remise2` + `typeRemise2` de ligne → ignorés (`remise="10%"`+`remise2="10%"` → 10 %, pas 20 %).
    - ajouter `typeRemise` (1ʳᵉ remise) → ne change rien (toujours 10 %).
    - `valeurRemise` (number) envoyé en entrée → ignoré (renvoie 0) ; c'est une **sortie** = montant de rabais calculé. `prixLotHTRemise` (sortie) = prix unitaire après remise.
  - ⇒ La doc Swagger (`easybeer-api-docs.json`) est auto-générée : elle liste ces champs mais SANS aucune sémantique ni enum (ni `ADDITIONNELLE`/`CASCADE`) → seul l'empirique + une commande créée par l'UI documentent le comportement.
  - **CE QUE FAIT VRAIMENT L'UI EASYBEER (décodé sur devis #2012, client CL000246, 2026-07-14)** — la référence à copier :
    - Chaque ligne reçoit **UNE remise `%` (champ `remise`) = la remise CIBLÉE produit si elle s'applique (elle REMPLACE la globale sur la ligne), SINON la remise globale 1 de la fiche.** Ex. Cola Carton18 → 10 % (ciblée) ; Pomme Carton12 → 12 % (globale).
    - Le total de la ligne = base − ce seul %, (+ droits d'accise pour un tarif « hors droits »). Vérifié : Cola ×4 = 140,40 − 14,04 (10 %) + 1,06 accise = 127,42 ; « dont remise » d'Easybeer = 14,04 = le 10 % SEUL.
    - **La « Remise 2 » en € (ex. 5 €) est AFFICHÉE sur la ligne (« + 5 € remise additionnelle ») mais N'EST PAS déduite du total.** Total remise du devis = somme des pourcentages seuls (14,04+17,55+5,62 = 37,21), le 5 €×3 n'y est pas. ⚠️ Quirk Easybeer (remise2 en € non appliquée) — touche AUSSI les commandes manuelles GOA. (Peut-être appliquée en %, ou à un autre stade — à confirmer avec GOA.)
    - ⇒ **Le bon envoi API = MIROIR de l'UI** : `remise` = ciblée-ou-globale1 (une seule), + repasser `remise2`/`typeRemise2` tels quels (pour parité d'affichage), et laisser Easybeer calculer le total. NE PAS additionner/fusionner (le `%` combiné testé plus tôt SUR-REMISE : 27 % au lieu de 10 %). Implémenté ainsi dans `enrichirLigneAvecRemises` (index.ts, 2026-07-14).
  - **`POST /parametres/remise` (`recupererRemiseBouteille`, body `ModeleRechercheRemise` {idClient,idProduit,idContenant,idStockProduit,quantite}) → `{map:{remise}}`** : renvoie la remise CIBLÉE produit (`listeRemises`) applicable (ex. Cola ×3 → `"10%"`, ×1 → `{}`). NE renvoie PAS la remise globale de fiche. Utile pour valider/calculer la remise ciblée côté serveur au lieu de la deviner.
  - **PRIORITÉ DES REMISES = CLIENT > SEGMENT (type client), PAS le max — vérifié en live (2026-07-14).** Le résolveur `recupererRemiseBouteille` renvoie **`10%`** (remise produit CLIENT) pour Cola même à ×24 où la remise produit SEGMENT `20%` (qté≥10) serait éligible → Easybeer applique la portée **CLIENT** en priorité, pas la plus avantageuse. Confirmé par une commande MANUELLE Easybeer (10 % appliqué auto). Idem au niveau commande : remise commande client prime sur celle du type. Règle générale : **le réglage du client individuel prime sur le segment ; le max ne joue qu'au sein d'une même portée ; le segment est un repli**. ⚠️ Piège : une commande créée VIA la plateforme montre toujours le même total qu'Easybeer parce qu'Easybeer **recopie** le `remise` qu'on lui envoie — ce n'est PAS une validation de la priorité (seul un devis créé MANUELLEMENT dans Easybeer, ou `recupererRemiseBouteille`, révèle la vraie priorité). Implémenté : `remisesCommande.ts` (`meilleureRemiseCibleeCommande`, portée `scope` posée par `allegerClient`), estimation front `web/lib/remises.ts`, tests `remisesCommande.test.ts` + E2E `verify-commande-coherence.ts` (compare au résolveur).
  - **Pas de double comptage au devis** (ligne 12 % → 12,636, jamais 24 %). ⚠️ **NON testé : le comportement à la FACTURATION** (devis→facture) — si Easybeer réapplique la remise de fiche à l'invoice, il y aurait double. À confirmer avec GOA/un vrai passage en facture avant prod.
