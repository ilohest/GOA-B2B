...
const INITIAL_PRODUCTS = [
  ...
  {
    id: "1l_framboise",
    name: "Kombucha Framboise - Hibiscus",
    format: "1L",
    packing: "Carton de 6",
    priceHT: 19.5,
    available: true,
...
const INITIAL_CLIENTS = [
  {
    id: "cli_biocoop",
    name: "Biocoop Épicerie Verte",
    codeEasybeer: "C00104",
    type: "GMS",
    delivery: "direct",
    franco: 250,
    email: "contact@biocoop-verte.fr",
    address: "12 Rue des Alouettes, Nantes",
  },
  {
    id: "cli_chope",
    name: "Le Bar de la Chope",
    codeEasybeer: "C00215",
    type: "CHR",
    delivery: "carrier",
    franco: 150,
    email: "contact@lachope.fr",
    address: "45 Place de la République, Angers",
  },
  {
    id: "cli_fine",
    name: "L'Épicerie Fine du Centre",
    codeEasybeer: "C00302",
    type: "CHR",
    delivery: "direct",
    franco: 150,
    email: "epicerie.centre@gmail.com",
    address: "8 Rue de la Paix, Saint-Nazaire",
  },
  {
    id: "cli_asso",
    name: "Festival Brasse-Temps",
    codeEasybeer: "C00489",
    type: "Evénement",
    delivery: "carrier",
    franco: 400,
    email: "bureau@brassetemps.org",
    address: "Hippodrome de Pornichet",
  },
];
...
const INITIAL_ORDERS = [
  {
    ...
    items: [
      { productId: "1l_pomme", quantity: 4, priceHT: 18.9 },
      { productId: "1l_gingembre", quantity: 6, priceHT: 19.5 },
      { productId: "35cl_framboise", quantity: 5, priceHT: 22.8 },
    ],
  ...
  },
  {
    ...
    items: [
      { productId: "35cl_gingembre", quantity: 3, priceHT: 22.8 },
      { productId: "35cl_cola", quantity: 2, priceHT: 24.0 },
    ],
    comment: "Merci d'étiqueter dluo visible.",
    subtotal: 116.4,
    deliveryFee: 18.0, // Sous le franco de 150
    totalTTC: 141.8, // (116.4 + 18) * 1.055
    easybeerSynced: false,
  },
];
...
const BottleIcon = ({ color, format }) => {
  ...
  return (
    ...
    >
      ...
      {/* Épaules bouteille */}
      <path
        d="M45,45 Q45,70 25,90 L25,185 Q25,195 35,195 L65,195 Q75,195 75,185 L75,90 Q55,70 55,45 Z"
        fill="#5C3A21"
      />
      {/* Liquide à l'intérieur (visible par transparence) */}
      <path
        d="M28,100 L72,100 L72,183 Q72,190 65,190 L35,190 Q28,190 28,183 Z"
        fill={color}
        opacity="0.85"
      />
      {/* Étiquette Kraft */}
...
export default function App() {
  ...
  const cartSummary = useMemo(() => {
    ...
    const deliveryFee = subtotalHT === 0 ? 0 : reachedFranco ? 0 : 25.0;
  ...
  const confirmOrder = () => {
    ...
    triggerToast(
      "Commande validée avec succès ! Transmise au backoffice.",
      "success",
    );
  };
  ...
  const generateEasybeerCSV = () => {
    ...
    if (unsynced.length === 0) {
      triggerToast(
        "Aucune nouvelle commande à exporter vers Easybeer !",
        "info",
      );
      return;
    }
    ...
    let csv =
      "ID_COMMANDE;CODE_CLIENT_EASYBEER;NOM_COMMERCE;DATE_CMD;MODE_LIVRAISON;SKU_PRODUIT;QUANTITE;PRIX_UNITAIRE_HT;FRAIS_PORT_HT;COMMENTAIRE\n";
  ...
  const markAllAsSynced = () => {
    ...
    triggerToast(
      "Commandes marquées comme 'Synchronisées' dans Easybeer !",
      "success",
    );
  };
  ...
  const downloadCSVFile = () => {
    ...
    link.setAttribute(
      "download",
      `import_easybeer_${new Date().toISOString().split("T")[0]}.csv`,
    );
  ...
  const handleCreateClient = (e) => {
    ...
    if (!newClient.name || !newClient.email || !newClient.address) {
      triggerToast(
        "Veuillez renseigner au moins le nom, l'email et l'adresse.",
        "error",
      );
      return;
    }
  ...
  const handleSimulateClientLogin = (client) => {
    ...
    setActiveTab("client");
    triggerToast(
      `Connexion automatique simulée en tant que : ${client.name}`,
      "info",
    );
  };
  ...
  return (
    <div className="min-h-screen bg-stone-50 text-stone-800 font-sans flex flex-col antialiased">
      ...
      <div className="bg-amber-950 text-stone-100 px-4 py-3 shadow-md flex flex-wrap items-center justify-between gap-3 border-b border-amber-800">
        <div className="flex items-center gap-2">
          ...
          <h1 className="font-semibold text-sm tracking-wide">
            B2B Kombucha & Intégration Easybeer
          </h1>
        </div>
              ...
              <div className="max-w-md mx-auto my-12 bg-white rounded-2xl shadow-xl border border-stone-100 overflow-hidden">
                <div className="bg-emerald-850 text-white p-6 text-center">
                  ...
                  <p className="text-emerald-100 text-xs mt-1">
                    Espace sécurisé de commande grossiste
                  </p>
                </div>
                ...
                <div className="p-6 space-y-4">
                  ...
                  <button
                    onClick={() => {
                      ...
                      setCart({});
                      triggerToast(
                        `Connecté en tant que ${selectedClient?.name || "Client"}`,
                      );
                    }}
                ...
                <div className="bg-white rounded-xl shadow-sm border border-stone-200/80 p-5 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                  <div>
                    ...
                    <p className="text-stone-500 text-xs flex flex-wrap gap-x-4 gap-y-1">
                      ...
                      <span>
                        • Type : <strong>{selectedClient?.type}</strong>
                      </span>
                      <span>
                        • Mode :{" "}
                        <strong className="text-amber-700">
                          {selectedClient?.delivery === "direct"
                            ? "Livraison directe par nos soins"
                  ...
                  <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="flex items-center justify-between">
                      <h3 className="text-lg font-bold text-stone-900">
                        Catalogue d'embouteillage
                      </h3>
                      <p className="text-xs text-stone-500">
                        Mise à jour en direct avec l'atelier
                      </p>
                    </div>
                      ...
                      <div className="border-b border-stone-200 pb-1">
                        <h4 className="font-bold text-stone-700 flex items-center gap-2">
                          ...
                          <span className="text-xs font-normal text-stone-400">
                            (Colisage par Carton de 6 bouteilles)
                          </span>
                        </h4>
                      </div>
                            ...
                            <div
                              ...
                              className={`bg-white rounded-xl border p-4 flex gap-4 transition-all relative overflow-hidden ${
                                product.available
                                  ...
                                  : "border-stone-100 opacity-60 bg-stone-50"
                              }`}
                            >
                    ...
                    <div className="space-y-3 pt-4">
                      <div className="border-b border-stone-200 pb-1">
                        <h4 className="font-bold text-stone-700 flex items-center gap-2">
                          ...
                          Format Individuel 35cL{" "}
                          <span className="text-xs font-normal text-stone-400">
                            (Nouveau Colisage par Carton de 12 bouteilles)
                          </span>
                        </h4>
                      </div>
                    ...
                    <div className="bg-white rounded-2xl border border-stone-200 shadow-sm p-5 sticky top-24 space-y-6">
                      <div className="border-b border-stone-100 pb-3">
                        ...
                        <p className="text-stone-400 text-xs">
                          Ajustez vos quantités si nécessaire
                        </p>
                      </div>
                      ...
                      <div className="space-y-2 bg-stone-50 p-4 rounded-xl border border-stone-100">
                        <div className="flex justify-between text-xs font-semibold">
                          <span className="text-stone-500">
                            Objectif Franco de Port :
                          </span>
                          <span className="text-emerald-700 font-bold">
                            {(selectedClient?.franco || 150).toFixed(2)} € HT
                          </span>
                        </div>
                        ...
                        <div className="w-full bg-stone-200 rounded-full h-3 overflow-hidden">
                          <div
                            ...
                            style={{
                              width: `${Math.min(100, (cartSummary.subtotalHT / (selectedClient?.franco || 150)) * 100)}%`,
                            }}
                          ></div>
                        </div>
                          ...
                          {cartSummary.reachedFranco ? (
                            ...
                            <span className="text-stone-500">
                              ...
                              <strong className="text-amber-700 font-bold">
                                {cartSummary.amountToFranco.toFixed(2)} € HT
                              </strong>{" "}
                              pour économiser les{" "}
                              <strong className="font-semibold">25,00 €</strong>{" "}
                              de frais de port.
                            </span>
                          )}
                        </div>
                              ...
                              >
                                <div className="flex-1 min-w-0 pr-2">
                                  ...
                                  <p className="text-stone-400 text-[10px]">
                                    {qty} x {prod.packing} @{" "}
                                    {prod.priceHT.toFixed(2)}€
                                  </p>
                                </div>
                                <span className="font-bold text-stone-800">
                                  {(prod.priceHT * qty).toFixed(2)} € HT
                                </span>
                              </div>
                            );
                          })
                      ...
                      <div className="border-t border-stone-100 pt-4 space-y-2 text-xs">
                        ...
                        <div className="flex justify-between text-sm font-bold text-stone-900 border-t border-stone-100 pt-2">
                          ...
                          <span className="text-emerald-800">
                            {cartSummary.totalTTC.toFixed(2)} €
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="space-y-1">
                        <label className="text-stone-500 font-semibold text-[10px] uppercase tracking-wider block">
                          Instruction ou Commentaire de livraison
                        </label>
                        <textarea
                          value={comment}
          ...
          <div className="space-y-6">
            ...
            <div className="bg-amber-900/10 border border-amber-900/20 rounded-2xl p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
              <div>
                ...
                <p className="text-amber-800 text-xs mt-1">
                  Gérez le catalogue instantané, consultez les commandes reçues
                  et exportez vers Easybeer.
                </p>
              </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 space-y-4">
                <div className="flex justify-between items-center">
                  ...
                  <span className="bg-amber-100 text-amber-800 font-bold px-2.5 py-1 rounded-full text-[10px]">
                    {orders.filter((o) => !o.easybeerSynced).length}{" "}
                    Non-transérées
                  </span>
                </div>
                    ...
                    >
                      ...
                      <div className="flex justify-between items-start border-b border-stone-100 pb-3 flex-wrap gap-2">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span
                              className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${
                                order.easybeerSynced
                                  ? "bg-emerald-100 text-emerald-800"
                                  : "bg-amber-100 text-amber-800"
                              }`}
                            >
                              {order.easybeerSynced
                                ? "Synchronisé Easybeer"
                                : "Prêt à Exporter"}
                            </span>
                          </div>
                          <p className="text-stone-400 text-[11px] mt-0.5">
                            Le {new Date(order.date).toLocaleString("fr-FR")}
                          </p>
                        </div>
                        ...
                        <div className="text-right">
                          ...
                          <p className="text-stone-400 text-[10px]">
                            ({order.subtotal.toFixed(2)} € HT +{" "}
                            {order.deliveryFee}€ Port)
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                        <div>
                          <p className="text-stone-400 font-semibold text-[10px] uppercase">
                            Client & Logistique
                          </p>
                          <p className="font-bold text-stone-800 mt-0.5">
                            {order.client?.name} ({order.client?.codeEasybeer})
                          </p>
                          <p className="text-stone-500 text-[11px] mt-0.5">
                            Mode :{" "}
                            <span className="underline">
                              {order.client?.delivery === "direct"
                                ? "Tournée Directe"
                                : "Transporteur"}
                            </span>
                            {order.client?.delivery === "direct" &&
                          ...
                          <div className="space-y-1 mt-1 max-h-24 overflow-y-auto">
                            {order.items.map((item, idx) => {
                              ...
                              return (
                                <p
                                  key={idx}
                                  className="text-stone-700 text-[11px] flex justify-between"
                                >
                                  <span>
                                    • {item.quantity}x{" "}
              ...
              <div className="space-y-4">
                <h3 className="text-lg font-bold text-stone-900">
                  Ruptures & Disponibilités
                </h3>
                <div className="bg-white rounded-2xl border border-stone-200 p-4 shadow-sm space-y-4">
                  <p className="text-xs text-stone-500 leading-relaxed">
                    Tentez l'expérience : désactivez un produit ici. Il
                    disparaîtra ou s'affichera immédiatement en "Rupture" sur
                      ...
                      >
                        <div className="flex-1 min-w-0 pr-2">
                          ...
                          <span className="text-stone-400 text-[10px]">
                            {product.format} — {product.packing}
                          </span>
                        </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-emerald-900/10 border border-emerald-900/20 rounded-2xl p-6">
              <h2 className="text-2xl font-bold text-emerald-950">
                Fichier Clients & Invitations Pro
              </h2>
              <p className="text-emerald-800 text-xs mt-1">
                Gérez vos comptes clients B2B, encodez de nouveaux commerces et
                générez des liens d'activation directs pour leur éviter l'oubli
                de mot de passe.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="bg-white rounded-2xl border border-stone-200 p-5 shadow-sm space-y-4">
                ...
                <p className="text-stone-500 text-xs">
                  Saisissez les informations de votre client pro pour générer
                  son lien unique.
                </p>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setNewClient({ ...newClient, franco: e.target.value })
                        }
                        className="w-full bg-stone-50 border border-stone-200 rounded-lg px-3 py-2 text-xs focus:ring-2 focus:ring-emerald-600 outline-none"
                        placeholder="150"
                      />
                ...
                <div className="space-y-3">
                  {clients.map((client) => {
                    ...
                    return (
                      <div
                        key={client.id}
                        className="bg-white rounded-xl border border-stone-200 p-5 shadow-sm space-y-3 flex flex-col justify-between"
                      >
                        {/* Infos client */}
                        ...
                        <div className="bg-stone-50 border border-stone-100 p-3 rounded-lg flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 text-xs">
                          <div className="flex-1 min-w-0">
                            ...
                            <span className="text-emerald-800 font-mono text-[11px] block truncate select-all">
                              {invitationLink}
                            </span>
                          </div>
        ...
        <div className="fixed inset-0 bg-stone-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-stone-100 space-y-4 animate-scaleUp">
            <div className="text-center space-y-2">
              ...
              <p className="text-stone-500 text-xs">
                Cette commande est ferme et **non annulable** conformément aux
                conditions de tournée logistique.
              </p>
            </div>
            ...
            <div className="bg-stone-50 p-4 rounded-xl border border-stone-100 text-xs space-y-1">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-bold text-stone-800">
                  {cartSummary.subtotalHT.toFixed(2)} €
                </span>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between border-t border-stone-200/80 pt-1.5 font-bold">
                <span className="text-stone-700">Total TTC :</span>
                <span className="text-emerald-800">
                  {cartSummary.totalTTC.toFixed(2)} €
                </span>
              </div>
        ...
        <div className="fixed inset-0 bg-stone-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl max-w-2xl w-full p-6 shadow-2xl border border-stone-100 space-y-4">
            <div className="flex justify-between items-start">
              <div>
                <h3 className="text-lg font-bold text-stone-900">
                  Génération du fichier passif Easybeer
                </h3>
                <p className="text-stone-500 text-xs mt-0.5">
                  Glissez-déposez ce fichier directement dans le module d'import
                  de votre interface Easybeer.
                </p>
              </div>
              <button
      ...
      <footer className="border-t border-stone-200 bg-white py-4 text-center text-xs text-stone-400">
        <p>
          Développé pour la Brasserie de Kombucha • Système de commande B2B V1
        </p>
      </footer>
    </div>
  );
